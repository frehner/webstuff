...
import edu.byu.isys413.afreh20.web.Action;
import edu.byu.isys413.afreh20.web.WebException;
	...
	public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (request.getParameter("guess") != null) {
			...
			if (guess < number) {
				...
				session.removeAttribute("number"); // forces it to be recreated
													// next time
				session.setAttribute("history", new LinkedList<String>()); // reset
																			// the
																			// history
			}// if
		}// if
